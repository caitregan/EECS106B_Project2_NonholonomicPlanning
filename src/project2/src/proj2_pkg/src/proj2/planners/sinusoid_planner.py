...
#from configuration_space import Plan, BicycleConfigurationSpace #CHANGE BACK
    ...
    def steer_y(self, start_state, goal_state, t0 = 0, dt = 0.01, delta_t = 2):
        ...
        while (a1_max - a1_min) > thres:
            ...
            if np.abs(G - delta_y) < thres: 
                ...
                #bin_search() #lower half
                a1_max = a1_mid
    '''
    def bin_search():=

        a1_mid = (a1_max - a1_min)/2
        integrand = lambda t: g(SinusoidPlanner.steer_alpha(t))*np.sin(omega*t) # The integrand to find beta
        beta1 = (omega/np.pi) * quad(integrand, 0, delta_t)[0]
